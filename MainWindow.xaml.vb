...
Class MainWindow
    ...
    Public Sub New()
        ...
        ' Set wall bounds based on the visible area where the player can move (center column image area)
        ' We use the imgRoom margin and size to estimate bounds. Adjust offsets for player sprite size.
        Dim leftOffset As Double = 10 ' left margin inside room area (matches imgRoom Margin)
        Dim topOffset As Double = 10
        Dim roomWidth As Double = imgRoom.ActualWidth
        Dim roomHeight As Double = imgRoom.ActualHeight
        If roomWidth = 0 Then roomWidth = 400 ' fallback
        If roomHeight = 0 Then roomHeight = 300

        WALL_LEFT = leftOffset
        WALL_TOP = topOffset
        WALL_RIGHT = leftOffset + roomWidth - imgPlayer.Width
        WALL_BOTTOM = topOffset + roomHeight - imgPlayer.Height
    ...
    Private Sub MoveLeft()
        ...
        Dim newLeft = imgPlayer.Margin.Left - 4
        If newLeft < WALL_LEFT Then newLeft = WALL_LEFT
        imgPlayer.Margin = New Thickness(newLeft, imgPlayer.Margin.Top, 0, 0)
    ...
    Private Sub MoveRight()
        ...
        Dim newLeft = imgPlayer.Margin.Left + 4
        If newLeft > WALL_RIGHT Then newLeft = WALL_RIGHT
        imgPlayer.Margin = New Thickness(newLeft, imgPlayer.Margin.Top, 0, 0)
    ...
    Private Sub MoveUp()
        ...
        Dim newTop = imgPlayer.Margin.Top - 4
        If newTop < WALL_TOP Then newTop = WALL_TOP
        imgPlayer.Margin = New Thickness(imgPlayer.Margin.Left, newTop, 0, 0)
    ...
    Private Sub MoveDown()
        ...
        Dim newTop = imgPlayer.Margin.Top + 4
        If newTop > WALL_BOTTOM Then newTop = WALL_BOTTOM
        imgPlayer.Margin = New Thickness(imgPlayer.Margin.Left, newTop, 0, 0)
    ...
    Private Sub UpdateRoomDisplay()
        ' Recalculate movement bounds based on the room image size so the player stays inside the view
    ...
    Private Function intersectsWith() As Boolean
        Dim imgPlayerRect As New Rect()
        imgPlayerRect.Location = imgPlayer.PointToScreen(New Point(0D, 0D))
        imgPlayerRect.Size = New Size(imgPlayer.Width, imgPlayer.Height)

        Dim myRectBox As New Rect()
        myRectBox.Location = box.PointToScreen(New Point(0D, 0D))
        myRectBox.Size = New Size(box.Width, box.Height)

        Dim doesIntersect As Boolean = imgPlayerRect.IntersectsWith(myRectBox)

        Return doesIntersect

        Console.WriteLine("Hit?" + intersectsWith().ToString)
        If intersectsWith() Then
            box.Visibility = Windows.Visibility.Collapsed
        End If
